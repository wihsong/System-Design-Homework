...
# copy medium binary file:
ln -s test_files/medium_binary.dat test.dat
new_files="$new_files test.dat command"

## run get command and save to file:
cat command | valgrind --leak-check=full --error-exitcode=1 --log-file=val.log ./memory >output.dat 2>/dev/null
mem_rc=$?
...
diff output.dat test.dat > diff.txt
diff_rc=$?
