...
cat command $test_file | env THROTTLER_READS=1 LD_PRELOAD="$source_dir/throttler.so" ./memory > output.txt 2>err.log
mem_rc=$?
...
diff test.txt test_files/medium_binary.dat > diff.txt
diff_rc=$?
