...
## return code
printf "get\ntest.dat\n" | strace -o syscalls.log -e trace=write ./memory > output.dat 2>err.log
mem_rc=$?
new_files="$new_files output.dat err.log syscalls.log"

write_calls=$(grep "write" syscalls.log | wc -l)

## Check each of the outputs:
## (1) memory's return code is 0
## (2) memory's stdout is the expected file
## (3) memory performed too many write system calls
...
if [[ $write_calls -gt 7813 ]]; then
    msg=$"${msg}Memory performed too many write calls"$'\n'
    rc=1
fi
